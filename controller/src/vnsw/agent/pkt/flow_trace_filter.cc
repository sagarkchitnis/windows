...
void FlowTraceFilter::SetFilter(bool enable, Address::Family family,
    ...
    if (src_addr_.is_v4()) {
        ...
        array<uint8_t, 16> addr_bytes = src_addr_.to_v6().to_bytes();
        array<uint8_t, 16> mask_bytes = src_mask_.to_v6().to_bytes();
        for (int i = 0; i < 16; i++) {
    ...
    if (dst_addr_.is_v4()) {
        ...
        array<uint8_t, 16> addr_bytes = dst_addr_.to_v6().to_bytes();
        array<uint8_t, 16> mask_bytes = dst_mask_.to_v6().to_bytes();
        for (int i = 0; i < 16; i++) {
            addr_bytes[i] = addr_bytes[i] & mask_bytes[i];
        }
...
static bool Ip6Match(const Ip6Address &ip1, const Ip6Address &ip2,
    ...
    array<uint8_t, 16> ip1_bytes = ip1.to_bytes();
    array<uint8_t, 16> ip2_bytes = ip2.to_bytes();
    array<uint8_t, 16> mask_bytes = mask.to_bytes();
